...
int grader(int newsockfd) { 
  ...
  int n = send(newsockfd, "File is received!\n", 18, 0);
  ...
  if(grader_message.size() != 0) {
    ...
    if(result_file.is_open()) {
      string read_line;
      while(getline(result_file, read_line)) { 
        if(send(newsockfd, read_line.c_str(), strlen(grader_result.c_str()), 0) == -1) {
...
int main(int argc, char* argv[]) {
        ...
        while(true) {
            ...
            request_count++;
            cout << "Grading request acccepted, server is grading!\n";
            cout << "Request count: " << request_count << "\n";
            grader(client_socket_fd);
        }
        close(socket_fd);
	return 0;
}